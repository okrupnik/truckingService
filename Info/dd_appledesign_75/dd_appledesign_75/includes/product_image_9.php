<?php
function product_image_9($productItems) {
    ?>
        ...
        <?php if ($productItems->productImage->imagesExists) : ?>
            ...
            <?php
                ...
                if (is_object($productItems->productImage->image))
                    $imgHtml = $productItems->productImage->image->displayMediaThumb('class=" bd-imagestyles-17"', false);
                else
                    $imgHtml = str_replace('<img', '<img class=" bd-imagestyles-17" ', $productItems->productImage->image);
            ?>
