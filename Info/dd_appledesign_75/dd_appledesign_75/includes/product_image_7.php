<?php
function product_image_7($productItems) {
    ?>
        ...
        <?php if ($productItems->productImage->imagesExists) : ?>
            ...
            <?php
                ...
                if (is_object($productItems->productImage->image))
                    $imgHtml = $productItems->productImage->image->displayMediaThumb('class=" bd-imagestyles-20"', false);
                else
                    $imgHtml = str_replace('<img', '<img class=" bd-imagestyles-20" ', $productItems->productImage->image);
            ?>
            <a class=" bd-productimage-7" href="<?php echo $productItems->productImage->link; ?>">
                <?php echo $imgHtml; ?>
            </a>
